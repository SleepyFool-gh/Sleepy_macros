Macro.add('p', {
    ...
    handler() {
        ...
        const postProcess = (pId) => this.self.postProcess(pId);
        ...
        setTimeout(() => postProcess(pId), 0);

    },



    // aux function, post process, wraps after wiki
    postProcess: function(pId) {
        const wout = $(`#p-macro-output-${pId}`).contents();
        const toWrap = [];
        const wrapUntil = (startIndex, pId) => this.wrapUntil(startIndex,pId);
    ...
    // aux function, wrap queuer, checks adjacent nodes to know which to wrap together
    wrapUntil: function(startIndex, pId) {
        const wout = $(`#p-macro-output-${pId}`).contents();
